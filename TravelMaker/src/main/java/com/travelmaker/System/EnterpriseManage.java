...
public class EnterpriseManage {
    ...
    public void AddUpdateDeleteEnterprise(char operation, long ID, String name, String password){
        ...
        if(operation == 'i'){
            try {
                ps = con.prepareStatement("INSERT INTO Enterpriseuser(ID, username, password) VALUES (?,?,?)");
                ps.setLong(1, ID);
                ps.setString(2, name);
        ...
        if(operation == 'u'){
            try {
                ps = con.prepareStatement("UPDATE `Enterpriseuser` SET `username`= ?, `password`= ? WHERE `ID` = ?");
        ...
        if(operation == 'd'){
            ...
            {
               try {
                ps = con.prepareStatement("DELETE FROM `Enterpriseuser` WHERE `ID` = ?");
                ps.setLong(1,ID);
                if(ps.executeUpdate() > 0){
                    JOptionPane.showMessageDialog(null, "Enterprise User Deleted");
                    }
    ...
    public void fillJtable(JTable table, String valueToSearch){
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT * FROM `Enterpriseuser` WHERE CONCAT(`username`)LIKE ?");
