...
public class UserManage {
    ...
    public void AddUpdateDeleteEnterprise(char operation, long ID, String name, String password){
        ...
        if(operation == 'i'){
            try {
                ps = con.prepareStatement("INSERT INTO Normaluser(ID, username, password) VALUES (?,?,?)");
                ps.setLong(1, ID);
                ps.setString(2, name);
        ...
        if(operation == 'u'){
            try {
                ps = con.prepareStatement("UPDATE `Normaluser` SET `username`= ?, `password`= ? WHERE `ID` = ?");
        ...
        if(operation == 'd'){
            ...
            {
               try {
                ps = con.prepareStatement("DELETE FROM `Normaluser` WHERE `ID` = ?");
                ps.setLong(1,ID);
                if(ps.executeUpdate() > 0){
    ...
    public void fillJtable(JTable table, String valueToSearch){
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT * FROM `Normaluser` WHERE CONCAT(`username`)LIKE ?");
